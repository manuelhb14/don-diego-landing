Create a single horizontal sprite strip for the Codex app digital pet `diego-rep` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Friendly Don Diego residential-club salesman and host; compact chibi human mascot with warm medium skin, small neat moustache, dark hair, cream guayabera-style shirt, charcoal vest, olive pocket square, tan trousers, small dark shoes, and one simple leather folio/tablet with no text; luxury San Miguel de Allende real-estate guide energy; approachable, calm, confident..
